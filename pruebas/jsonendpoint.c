#include <string>  // If using std::string
#include <vector>  // If using std::vector
#include <iostream>// If using std::ostream or std::istream
...
Json::Value crearDatos(std::string segment) {
	...
	// string segment = "api/getcharacter";
	...
	// curl_easy_setopt(curl, CURLOPT_URL, baseUrl + segment);
	...
	// raiz["character_name"] = characterName;
	// raiz["id"] = id;
	// return raiz;
	// return responseBody;
}
...
int main() {
	...
	// Json::Value resultado = crearDatos("http://127.0.0.1:8000/api/getcharacter");
